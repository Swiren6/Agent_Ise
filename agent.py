...
class SQLAssistant:
    def __init__(self):
        self.cache_file_path = os.path.abspath("question_cache2.json")
    ...
    def ask_question(self, question: str) -> tuple[str, str, float, int]:
        ...
        if question in self.cache_data:
            ...
            self.save_history(question, cached["sql"], cached["response"])  # OK
        ...
        try:
            ...
            if result and result.strip() not in ["[]", ""] and not ("0 rows" in result):
                ...
                if len(lines) > 1:  # Au moins une ligne de données
                    ...
                    self.save_cache()
                    # self.save_history(question, sql_query, formatted)
    ...
    def save_cache(self):
        try:
            ...
            print(f"✅ Cache sauvegardé dans {self.cache_file_path}")
        except Exception as e:
